...
contract Blocklist {
  ...
  function addToBlocklist(address account) external returns (bool) {
    require(operators[msg.sender], "999");

    return _addToBlocklist(account);
  }

  function batchAddToBlocklist(address[] calldata accounts) external {
    require(operators[msg.sender], "999");
  ...
  function batchRemoveFromBlocklist(address[] calldata accounts) external {
    require(operators[msg.sender], "999");
  ...
  // Operators
  function setOperator(address account, bool active) public {
    require(operators[msg.sender], "999");
