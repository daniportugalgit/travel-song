...
import "./KingdomBridgeSignature.sol";
import "../../ECDSA.sol";

contract KingdomBridgeSigGuardian is KingdomBridgeSignature {
  ...
  function _hasCorrectNonce(address playerAddress, uint256 nonce) internal view returns (bool) {
    require(
      lastNonceUpBlock[playerAddress] == 0 || block.number > lastNonceUpBlock[playerAddress] + 1,
      "INV_NONCE_UPD"
    );
